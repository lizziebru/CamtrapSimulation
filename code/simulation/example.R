...
path2 <- pathgen(5e3, kTurn=2, kCor=TRUE, pTurn=1, 
                ...
                xlim=c(0,10), wrap=TRUE)
# takes really long for high speeds - why?
...
# would be good to have some way to visualise how the measured speeds relate to what it's meant to be
...
# would be nice to see how this relationship changes as you up the speed

# make function to do everything with varying speed as an input:

varying_speeds <- function(speed) {
  path <- pathgen(5e3, kTurn=2, kCor=TRUE, pTurn=1, 
...
speeds <- seq(from = -3, to = 3, by = 0.5)

# run the simulation on each speed
plots <- lapply(speeds, varying_speeds)

# arrange all the plots in one panel
m <- marrangeGrob(plots, nrow = 7, ncol = 2)

ggsave(filename = "high_speeds.png", plot = m, path = "plots", width = 10, height = 18)
...
# but also this isn't really showing us that CTs miss high speeds - it's more just showing us how high speeds get underestimated
