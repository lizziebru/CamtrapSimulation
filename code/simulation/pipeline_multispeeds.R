...
seq_dat <- function(speed_parameter, step_no, size) { 
  xlim <- c(0,40)
  #ylim <- c(2,16)
  path <- pathgen(n=step_no, 
                   ...
                   #ylim=ylim, #--> ylim = xlim
                   wrap=TRUE)
  dz <- data.frame(x=20, y=10, r=10, th=1.2, dir=0) # set radius to 10m and theta to 1.65 - based on distributions of radii & angles in regent's park data - then changed theta to 1.2 bc looked too big
  # make plot
  # plot_wrap(path, lineargs = list(col="grey"))
  # plot_dzone(dz, border=2)
  
  # generate position data
  # use alternative versions of sequence_data function depending on if small or large
...
#seq_dats <- sapply(speed_parameter, seq_dat, step_no = step_no, size = size) 
# --> too slow


# mcsapply
# forking method of parallelisation using mclapply family
...
seq_dats <- mcsapply(speed_parameter, seq_dat, mc.cores = 4, step_no = step_no, size = size)
...
folder_name <- paste("plots/", exp(speed_parameter[1]), "-", exp(speed_parameter[length(speed_parameter)]), "_", step_no, "_", size, sep = "")
...
ggsave(filename = paste(folder_name, "/bias1_1.png", sep = ""), plot = bias1_arranged, height = 10, width = 30)
...
bias2_1_df <- data.frame(real_mean = c(rep(real_mean, length(2))),
                         ...
                         type_of_count = c(rep("single frames", length(singles)), rep("zero frames", length(zeros))))
...
bias2_1_plot <- ggplot(bias2_1_df, aes(x = real_mean, y = count, colour = type_of_count))+
  ...
  labs(x = "mean realised speed (m/s)", y = "count")+
  facet_grid(rows = vars(type_of_count))
bias2_1_plot

png(file= paste(folder_name, "/bias2_1.png", sep = ""),
    width=700, height=650)
bias2_1_plot
dev.off()


## PLOT: bias2_2.png --> an alternative to raw count
# number of single or zero frames divided by total no. of points for each mean realised speed
singles_prop <- c()
zeros_prop <- c()
for (i in 1:length(speed_parameter)){
  s <- seq_dats[,i]
  singles <- s$n_singles[1]/s$n_points[1]
  zeros <- s$n_zeros[1]/s$n_points[1]
  singles_prop <- c(singles_prop, singles)
  zeros_prop <- c(zeros_prop, zeros)
}
...
png(file= paste(folder_name, "/bias2_2.png", sep = ""),
    width=700, height=650)
bias2_2_plot
dev.off()
...
# linking no. of single & zero frames to the resulting discrepancy between estimated speed and speed parameter would be helpful to demonstrate how the biases are affecting our way of estimating average speed of movement 
...
# previous plotting - to look through and double check but probs don't need anymore
...
# PLOTS FOR DISTRIBUTION OF REALISED SPEEDS - for when was struggling to understand why had massively high realised speeds --> turns out was bc of the speedSD value
...
# real_spds_distr
# dev.off()
# some of the realised speedswere way too high - have discussed with M now though so don't need anymore
...
## detection probability plot from before (M & C didn't think it was that useful)
## PLOT: bias1_3.png
...
# 
# 
# png(file=paste(folder_name, "/bias1_3.png", sep = ""),
#     width=700, height=600)
# det_probs_plot
# dev.off()
