...
seq_dats <- mcsapply(speed_parameter, seq_dat, mc.cores = 4, step_no = 5e5, size = 0, xlim = c(0,40), speedSD = speedSD, pTurn = pTurn, speedCor = speedCor, kTurn= kTurn)
...
plot_sim(speed_parameter[1], step_no = 5e5, size = 0, xlim = c(0,40), speedSD = speedSD, pTurn = pTurn, speedCor = speedCor, kTurn= kTurn) # plot just the first one
dev.off()
